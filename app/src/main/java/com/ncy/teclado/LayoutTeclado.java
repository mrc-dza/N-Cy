...
public class LayoutTeclado {
    ...
    private int altoFilaBase;
    
    private boolean mostrarBarraBase = true;

    public LayoutTeclado(Context contexto, RepositorioConfiguracion repositorioConfig, EstadoTeclado estadoTeclado, GestorTema gestorTema) {
        this.contexto = contexto;
        this.repositorioConfig = repositorioConfig;
        this.estadoTeclado = estadoTeclado;
        this.gestorTema = gestorTema;
        this.altoFilaBase = gestorTema.obtenerTemaActivo().getAlturaFilaBasePx();
    }
    ...
    public int calcularAltoReferencia(int anchoPantalla) {
        ...
        return calcularAltoDesdeFilas(filasRef);
    }
    ...
    public void construirTeclado(int anchoPantalla, ModoTeclado modoActual) {
        ...
        if (altoReferencia == 0) {
            ...
            altoReferencia = calcularAltoDesdeFilas(letrasCrudas);
    ...
    private int calcularAltoDesdeFilas(List<List<Tecla>> filas) {
        float suma = 0f;
        for (List<Tecla> fila : filas) {
            if (!fila.isEmpty()) suma += fila.get(0).obtenerMultiplicadorAlto();
        }
        return suma == 0f ? 0 : Math.round(altoFilaBase * suma);
    }
