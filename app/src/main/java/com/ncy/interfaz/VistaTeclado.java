...
public class VistaTeclado extends LinearLayout {
    ...
    public VistaTeclado(Context contexto, ManejadorEntrada manejador,
                        ...
                        ReconstruibleTeclado callbackReconstruccion, GestorTema gestorTema) {
        super(contexto);
        ...
        setBackgroundColor(gestorTema.obtenerTemaActivo().getColorFondoGeneral());

        RepositorioConfiguracion repoCfg = new RepositorioConfiguracion(contexto);
        this.layout         = new LayoutTeclado(contexto, repoCfg, estado, gestorTema);
        this.vistaTeclasXml = new VistaTeclasXml(contexto, manejador, estado, layout, gestorTema);

        this.vistaTeclasXml.setListenerModo(this::reconstruirYRedibujar);
    ...
    public void reconstruirYRedibujar() {
        ...
        if (modo == ModoTeclado.PORTAPAPELES) {
            ...
            if (contenedorPortapapeles.getChildCount() == 0) {
                new ConstructorVistaPortapapeles(getContext(), manejador, estado, manejadorPortapapeles, layout, gestorTema)
