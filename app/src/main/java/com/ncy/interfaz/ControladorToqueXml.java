...
public class ControladorToqueXml {
    ...
    public void liberarRecursos() {
        if (manejadorRepeticion != null) manejadorRepeticion.detener();
    ...
    public boolean procesarToque(MotionEvent evento) {
        ...
        switch (evento.getAction()) {
            case MotionEvent.ACTION_DOWN -> { return procesarActionDown(xActual, yActual); }
            case MotionEvent.ACTION_MOVE -> { return procesarActionMove(xActual, yActual); }
        ...
        if (!gestoEjecutado && !controladorBarra.isArrastrando()) {
            ...
            if (Math.abs(dx) > 30 && Math.abs(dx) > Math.abs(dy)) {
                if (estado.obtenerModoActual() != ModoTeclado.PORTAPAPELES && teclaPresionadaInicialmente.obtenerY() == 0) {
                    manejadorRepeticion.detener();
                    // Usamos el gestor inyectado
                    int altoFila = gestorTema.obtenerTemaActivo().getAlturaFilaBasePx();
